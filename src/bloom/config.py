...
def extract_values_from_env(config: dict, allow_extend: bool = False) -> dict:
    ...
    for k, v in os.environ.items():
        ...
        if (
            k.lower() in [f"{k}_FILE".lower() for k in config.keys()]
...
def extract_values_from_file(
    filename: str, config: dict, allow_extend: bool = False, env_priority: bool = True
) -> dict:
    """function that extrat key=value pairs from a file
    Parameters:
        ...
        for line in file:
            ...
            if len(split) == split_succeed:
                ...
                if (
                    k.lower() in [f"{k}_FILE".lower() for k in config.keys()]
                    and (
                        k.removesuffix("_FILE").lower() in config.keys() or allow_extend == True
                    )
                    and Path(v).is_file()
                ):
...
class Settings(BaseSettings):
    ...
    def __init__(self):
        ...
        bloom_config = os.getenv(
            "BLOOM_CONFIG", Path(__file__).parent.parent.parent.joinpath(".env")
        )
        ...
        if Path(bloom_config).exists():
            extract_values_from_file(
                bloom_config, self.__dict__, allow_extend=False, env_priority=True
            )
        else:
